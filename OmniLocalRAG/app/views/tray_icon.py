    ...
    def run(self) -> None:
        ...
        try:
            if self._action == "load":
                ...
                llm_ok = llm.load()
                if llm_ok:
                    self.finished_with_result.emit(True, "模型已加载：Gemma 已就绪。", "ready")
                    return
                self.finished_with_result.emit(False, f"模型加载失败：Gemma: {llm.last_error or 'load failed'}", "error")
                return

            if self._action == "unload":
                LLMManager().unload(reason="manual")
                gc.collect()
                self.finished_with_result.emit(True, "模型已释放：Gemma 已卸载。", "unloaded")
                return
...
class TrayIcon(QSystemTrayIcon):
    ...
    def _build_menu(self) -> None:
        ...
        act_prefs = menu.addAction("偏好设置")
        self._act_load = menu.addAction("加载模型")
        self._act_free = menu.addAction("立即释放内存")
        act_editor = menu.addAction("打开知识库编辑器")
        menu.addSeparator()
