...
echo   Uses CUDA PyTorch + llama-server (no llama-cpp-python wheel)
echo ============================================================
echo.
...
if "%CUDA_MAJOR%"=="13" (
    ...
    if %CUDA_MINOR% GEQ 8 (
        ...
        echo [ERROR] Detected CUDA %CUDA_VER_FULL%. PyTorch CUDA wheels require driver support for CUDA 12.6+.
...
echo.
echo [4/8] Installing PyTorch %CUDA_TAG% build ...
...
echo   Run: "%PY%" main.py
echo   Embedding and llama-server are configured for CUDA.
echo ============================================================
pause & exit /b 0
